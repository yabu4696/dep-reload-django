...
from config.tasks import lens_form_celery, lens_reload_celery, add
    ...
    if not request.user.is_superuser:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                lens_form_celery.apply_async()
            return redirect('ca_lens:index')
        else:
            form = WantoitemForm()
            return render(request, 'ca_lens/form.html',{'form':form})
...
def reload(request):
    if not request.user.is_superuser:
        ...
        if request.method == 'POST':
            ...
            lens_reload_celery.apply_async(item_pks)
            return redirect('ca_lens:reload')
        else:
            items = Wantoitem.objects.all().order_by('maker_name')
            return render(request, 'ca_lens/reload.html', {'items':items})
